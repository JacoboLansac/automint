...
def main():
        ...
        if network.show_active() == 'development':
            if not MRCRYPTO:
                ...
                account.deploy(
                    ...
                    "MRCRYPTO",  # _name,
                    "MRCRYPTO",  # _symbol,
                    "sdadsfa",  # _initBaseURI,
                    "asdfasdfa",  # _initNotRevealedUri"""
                )
            return MRCRYPTO[-1]
        elif network.show_active() == 'polygon-main-alchemy':
    ...
    pkey = config['wallets']['burn-account1']
    print("burn account", pkey)
    account = accounts.add(private_key=pkey)
    print(f"{account} balance: {account.balance()}")

    mrcrypto = get_contract()

    def mint():
        amount = 8
        matic_price = 50000000000000000000
        payable = Web3.toWei(amount * matic_price, 'ether')
        tx = mrcrypto.mint(amount, {
            "from": account,
            "value": payable,
            "priority_fee": "300 gwei",
            "base_fee": "20 wei"
        })
        tx.wait()
        print(tx)
    ...
    while True:
        try:
            whitelistOn = mrcrypto.whitelistOn()
            if whitelistOn:
                print(f"[{round(time.time())}]: Whitelist still On")
                time.sleep(1)
            else:
                print(f"Attempting to mint")
                mint()
                break
